...
ctdfile = dir('/Volumes/scienceparty_share/ctd/processed100/*.mat');
...
for i=1:length(datafile) 
   ...
   elseif(l==3)
    icast(i) = str2double(datafile(i).name(13:15));
   end
       
end


newcast = find(icast>str2double(ctdfile(end).name(13:15)));
if(~isempty(newcast))
    for i=1:length(newcast)
        load_hex_mendo100(newcast(i));
    end
    initialize
else
    disp('No new file found.')
    disp(['Latest file: ',ctdfile(end).name])
    disp('--------------------------------------------')
end
%{
